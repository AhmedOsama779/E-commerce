...
export const sendCode = async (req, res, next) => {
    ...
    if (!user) {
        return next(new Error('please register', { cause: 400 }))
    }
    const forgetCode = nanoId()
    // const message = `<p>Your OTP is ${forgetCode} </p>`
    ...
    res.status(201).json({ message: "Please your mail", updatedUser })
...
export const resetPassword = async (req, res, next) => {
    ...
    const user = await userModel.findOne({ email })
    if (!user) {
        return next(new Error('please if enter a valid email', { cause: 400 }))
    ...
    //     password: pkg.hashSync(newPassword, +process.env.SALT_ROUND),
    //     forgetCode: null
    // }, {
    //     new: true
    // })
